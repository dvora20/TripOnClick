...
export default function FilterComp2() {
    ...
    useEffect(() => {
        ...
        setNotUsed(attractions2.filter(x => !choosedAttractions2.includes(x)));
    ...
    const Scheduling = async () => {
        ...
        let [start, limit] = [startDate, new Date(endDate)];
        ...
        for (let i = 0; i < length; i++) {
            ...
            //for saturday
            if ((dayOfWeek === 6) && (start.getDate() < end.getDate())) {
            ...
            let waterFlag = 0;
            let cultureFlag = 0;
            console.log(extremeFlag, waterFlag)
            ...
            if (attractions.length > 24) {
                getMeRandomElements(attractions, 25)
                attractions = result
            }
            ...
            while (start < limit && start.getHours() < limit.getHours()) {
                var dayOfWeek = start.getDay();
                let strAttractions = attractions.map((attraction => attraction.Address.Street + " + " + attraction.Address.Number + " + " + attraction.Address.City))
                ...
                strAttractions = fitAttractions.map((attraction => attraction.Address.Street + " + " + attraction.Address.Number + " + " + attraction.Address.City))
                if (fitAttractions.length > 24) {
                    getMeRandomElements(fitAttractions, 25)
                    fitAttractions = result
                }
                ...
                var getMeRandomElements2 = function (sourceArray, length) {
                    for (var i = 0; i < length; i++) {
                        result2.push(sourceArray[Math.floor(Math.random() * sourceArray.length)]);
                ...
                if (attractions.length > 24) {
                    getMeRandomElements2(attractions, 25)
                }
